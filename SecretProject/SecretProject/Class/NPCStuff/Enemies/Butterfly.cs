...
namespace SecretProject.Class.NPCStuff.Enemies
{
    public class Butterfly : Enemy
    {
        ...
        public override void Draw(SpriteBatch spriteBatch, GraphicsDevice graphics, ref Effect effect)
        {
            ...
            this.NPCAnimatedSprite[0].DrawAnimation(spriteBatch, new Vector2(this.Position.X - this.NPCRectangleXOffSet - 8 + this.FlutterOffset.X, this.Position.Y - this.NPCRectangleYOffSet - 8 + this.FlutterOffset.Y), .5f + (Game1.Utility.ForeGroundMultiplier * ((float)this.NPCAnimatedSprite[0].DestinationRectangle.Y)), this.Rotation);
        }
